...
output <- "./output/wheat_IND/"
dir.create(output, showWarnings = FALSE)

#read data
mydata <- read_csv("./input/IND/climmob_india.csv")



#keep only wheat data 
mydata <- mydata[mydata$crop == "wheat",]
...
#remove items tested in only 1 season
rm_item <- as.data.frame(table(mydata$variety_a, mydata$season))
rm_item[rm_item > 0] <- 1
rm_item <- aggregate(rm_item[,"Freq"], by=list(rm_item[,"Var1"]), sum)
rm_item <- rm_item[rm_item$x<2,]
for(i in unique(rm_item[,1])){
  mydata$variety_a <- ifelse(mydata$variety_a == toString(i), NA, mydata$variety_a)
  mydata$variety_b <- ifelse(mydata$variety_b == toString(i), NA, mydata$variety_b)
  mydata$variety_c <- ifelse(mydata$variety_c == toString(i), NA, mydata$variety_c)
}
...
names(mydata)
overall <- mydata[,c(2:4,10:12)]
sum(is.na(overall[5:6]))

yield <- mydata[,c(2:4,13:15)]
...
mydata <- cbind(mypca[,c("PC1","PC2","PC3")], mydata[,c(2:4,10:12)], covar)
n <- nrow(mydata) 
head(mydata)

#generate PL grouped rankings
G <- grouped_rankings(as.PL(mydata, local = F, additional.rank = F ), seq_len(n)) 
#add to mydata 
...
lambda <- seq(0.030, 0.040, 0.001) #to smooth weigths

#combinations lambda x covariates 
CLC <- rbind(rep(climcov, each = length(lambda)), rep(lambda, times = length(climcov)))

# #s <- c(792L, 155L, 609L, 640L, 6L, 359L, 357L, 627L, 682L, 513L)
# #vars <- CLC[1,s]
# #CLC <- cbind(CLC[,s], CLC[,ncol(CLC)])
# #CLC[2,] <- 0.040
...
blockedfolds[1,2:4] <- unlist(crossvalidation_PLTE(as.formula(c("G ~ P1")),
